...
namespace WebAPI2.Models
{
    public class TripProcessing
    {
        ...
        public async Task<bool> SendOffer(int toUID, int RouteID,DateTime atTime)
        {
            ...
            try
            {
                try
                {
                    ...
                    {
                        var RoutePointLocation = dbo.RoutePoints.First(req => req.RoutePointId== RouteID);
                ...
                catch (Exception)
                {
                    ...
                    await MessagesProcessing.SendSysMessage(App.UID, toUID, "", 1);
                }
        ...
        public bool LoadOffers(long CurrentUserTime) {
            var dbo = new AppDbDataContext();
            try
            {
                try
                {
                    var time = new DateTime(CurrentUserTime);
                    var trips = dbo.Trips.Where(req => (req.DriverUID == App.UID || req.PassengerUID == App.UID) && req.PlannedStartTime > time  );
                    bool isUserDriver = dbo.Users.First(req => req.UID == App.UID).isDriver.Value;
                    foreach (var trip in trips) {
                        ...
                        NextOffer.StartTime = trip.PlannedStartTime.Ticks;
        ...
        public async Task<bool> AcceptOffer(int OfferID)
        {
            ...
            try
            {
                try
                {
                    ...
                    dbo.SubmitChanges();
                    await MessagesProcessing.SendSysMessage(App.UID, companion, "", 5);
                    return true;
                }
                catch (Exception)
                {
        ...
        public async Task<bool> RejectOffer(int OfferID)
        {
            var dbo = new AppDbDataContext();
            try
            {
                try
                {
                    ...
                    int companion = isUserDriver ? trip.PassengerUID : trip.DriverUID;
                    await MessagesProcessing.SendSysMessage(App.UID, companion, "", 3);
